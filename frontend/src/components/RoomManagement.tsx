...
const RoomManagement: React.FC<UserProps> = ({ user }) => {
  ...
  const [roomTypeFilter, _setRoomTypeFilter] = useState<string>('Lecture');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch rooms
        const roomsRes = await api.get('/tbl_rooms'); // Replace with your endpoint
        setRoomOptions(roomsRes.data ?? []);

        // Fetch buildings
        const buildingsRes = await api.get('/tbl_buildings'); // Replace with your endpoint
        setBuildingOptions(
          buildingsRes.data?.map((b: any) => ({ id: b.building_id, name: b.building_name })) ?? []
        );

        // Fetch exam occupancy
        const examsRes = await api.get('/tbl_examdetails'); // Replace with your endpoint
        ...
        console.error('Error fetching room/building data:', err.message);
      }
    };
    fetchData();
  }, []);
      ...
      <div className="rm-card">
        ...
        <div className="rm-field">
          ...
          <Select
            ...
            value={
              selectedBuilding
                ? { value: selectedBuilding, label: buildingOptions.find((b) => b.id === selectedBuilding)?.name }
                : null
            }
      ...
      {occupancyModal.visible && occupancyModal.roomId && (
        <div className="rm-modal-overlay" onClick={() => setOccupancyModal({ visible: false, roomId: null })}>
          <div className="rm-modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <button className="rm-close-btn" onClick={() => setOccupancyModal({ visible: false, roomId: null })}>
              Close
            </button>
          </div>
        </div>
