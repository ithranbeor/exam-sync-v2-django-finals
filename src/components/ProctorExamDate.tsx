...
const ProctorExamDate = () => {
  ...
  useEffect(() => {
    const fetchUserRoleAndCollege = async () => {
      ...
      // ✅ Step 1: Get internal user_id from tbl_users
      ...
      if (userRowError || !userRow) {
        console.error("Error fetching internal user_id:", userRowError?.message);
        return;
      }

      const realUserId = userRow.user_id;

      // ✅ Step 2: Get scheduler role & college
      const { data: roleRow, error: roleError } = await supabase
        .from("tbl_user_role")
        .select("role_id, college_id")
        .eq("user_id", realUserId)
        .eq("role_id", 3) // only scheduler role
        .maybeSingle();

      if (roleError) {
        console.error("Error fetching role/college:", roleError.message);
        return;
      }

      if (roleRow?.role_id === 3) {
        setIsScheduler(true);

        const college = roleRow.college_id ?? null;
        if (college) {
          setSchedulerCollege(college);
          // ❌ remove setFilters here
        }
      }
    };
  ...
  useEffect(() => {
    const fetchExamPeriods = async () => {
      const { data, error } = await supabase.from('tbl_examperiod').select('*');
      if (!error) setExamPeriods(data);

      const { data: terms, error: termError } = await supabase.from('tbl_term').select('*');
      if (!termError) {
  ...
  const handleDayClick = (date: Date) => {
    if (!isScheduler) return; // only schedulers can pick

    // Only allow selection if date is inside an exam period
    const exists = examPeriods.some((ep) => {
    ...
    setSelectedDates((prev) => {
      const already = prev.some((d) => d.toDateString() === date.toDateString());
      return already
  ...
  if (!schedulerCollege) {
    alert("No college assigned to this scheduler.");
    return;
  }
  ...
  try {
    ...
    const { data: updatedRows, error: updateError } = await supabase
      .from("tbl_examperiod")
      .update({ college_id: schedulerCollege })
      .in("start_date", dateStrings) // exact match
      .select("*");

    if (updateError) {
      console.error("Error updating exam periods:", updateError);
      alert("Failed to update exam period(s): " + updateError.message);
      return;
    }
    ...
    alert(
      `Successfully updated ${updatedRows?.length ?? dateStrings.length} exam period(s) to your college.`
    );
    setSelectedDates([]);
  } catch (err) {
    console.error("Unexpected error in handleSaveDates", err);
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const events = examPeriods.filter((ep) => {
      ...
      // ✅ Exact match: only show events where this day is the exam date
      const examDate = new Date(ep.start_date);
      return date.toDateString() === examDate.toDateString();
    });
    ...
    calendarCells.push(
      <div
        key={`day-${day}`}
        className={cellClass}
        onClick={() => handleDayClick(date)} // scheduler can click
      >
        <span className="day-number">{day}</span>
        {events.length > 0 && (
          <div className="event-details">
            {/* ✅ show color badges per college */}
    ...
    <div className="dashboard">
      <div className="dashboard-header">
        ...
        <div className="filters">
          ...
          {/* Scheduler: lock to their college */}
          {isScheduler ? (
        ...
        {isScheduler && (
          <div className="save-section">
            <button type="button" className="save-button" onClick={handleSaveDates}>
              Save Selected Dates
            </button>
          </div>
        )}
